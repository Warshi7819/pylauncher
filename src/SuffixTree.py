...
class Tree:
    ...
    def addString(self, str, id):
        ...
        if not str == "":
            ...
            # Mark end of string
            strArray.append(self.endChar)

            for i in range(0, len(strArray) - 1):
                curChar = strArray[i]
                if curChar != self.endChar:
    ...
    def searchAndInsert(self, subString, id, curNode):
        ...
        for i in range(0, len(subString)):
            if i == len(curNode.str):
                ...
                if subString[i] == self.endChar:
                    if not curNode.str[i] == self.endChar:
    ...
    def dotGraph(self):
        ...
        self.traverseAndDot(fp, self.root, "node_0")
    ...
    def traverseAndDot(self, fp, node, dotText):
        ...
        nodeName = "node_%d" % (node.id)

        if not node.id in self.nodes:
            self.nodes[node.id] = 0
            fp.write("node_%d [shape=record,style=solid,label=\"%d\\n%s\\n%s\"];\n" % (node.id,
                                                                                       node.id,
                                                                                       "".join(node.str),
                                                                                       ",".join(node.matches)))

        output =  "%s -> %s" % (dotText, nodeName)
        fp.write("%s;\n" % output)
...
if __name__ == "__main__":
    ...
    print("  indexed %d items in : %d secs" % (len(testSet), (end - start)))
